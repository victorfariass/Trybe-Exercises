...
-- 3. Monte uma query que exiba a média de duração por classificação indicativa ( rating ) dos filmes cadastrados na tabela 
-- sakila.film . Os resultados devem ser agrupados pela classificação indicativa e ordenados da maior média para a menor.
SELECT rating, AVG(length) AS media_de_duracao FROM sakila.film
GROUP BY rating
...
SELECT district, COUNT(*) AS quantidade_de_enderecos FROM sakila.address
GROUP BY district
ORDER BY quantidade_de_enderecos DESC;
...
FROM sakila.film
GROUP BY rating
HAVING duracao_media BETWEEN 115.00 AND 121.50
...
SELECT rating, SUM(replacement_cost) AS soma_substituicoes
FROM sakila.film
GROUP by rating
HAVING soma_substituicoes > 3950.50
ORDER BY soma_substituicoes ASC;