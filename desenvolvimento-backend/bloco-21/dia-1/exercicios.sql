...
SELECT SUM(SALARY) AS salario_total FROM hr.employees;
...
SELECT JOB_ID, COUNT(*) AS PESSOAS_PROGRAMADORAS FROM hr.employees
GROUP BY JOB_ID
HAVING JOB_ID = 'IT_PROG';
-- 7. Escreva uma query que exiba a quantidade de dinheiro necessária para efetuar o pagamento de cada profissão ( JOB_ID ).
SELECT JOB_ID, SUM(SALARY) AS TOTAL_SALARIAL FROM hr.employees
GROUP BY JOB_ID;
...
GROUP BY JOB_ID
HAVING JOB_ID = 'IT_PROG';
-- 9. Escreva uma query que exiba em ordem decrescente a média salarial de todos os cargos, exceto das pessoas
...
COUNT(*) AS QUANTIDADE_DE_FUNCIONARIOS FROM hr.employees
...
-- 14. Escreva uma query que exiba as seguintes informações de cada funcionário: id , primeiro nome e dia do mês
-- no qual foi contratado (exiba somente o dia).
SELECT EMPLOYEE_ID, FIRST_NAME, DAY(HIRE_DATE) HIRE_DAY FROM hr.employees;
-- 15. Escreva uma query que exiba as seguintes informações de cada funcionário: id , primeiro nome e mês no qual
-- foi contratado (exiba somente o mês).
...
SELECT FIRST_NAME, LAST_NAME, DATEDIFF(CURRENT_DATE(), DATE(HIRE_DATE)) AS DIAS_TRABALHADOS FROM hr.employees;
